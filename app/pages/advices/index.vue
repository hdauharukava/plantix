<template>
  <div class="flex flex-col items-center px-8 sm:px-4 md:px-6 lg:px-8 py-6 md:py-8">
    <div class="w-full max-w-5xl mb-6 md:mb-8">
      <div>
        <h1 class="font-poppins font-bold text-xl md:text-2xl lg:text-3xl text-[#1e1e1e]">
    ...
    <div class="w-full max-w-5xl lg:hidden mb-4">
      ...
      >
        <div class="flex items-center space-x-3">
          <svg class="w-5 h-5 text-[#90a88c]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <div class="w-full max-w-5xl">
      <div class="flex flex-col lg:flex-row gap-6 md:gap-8 mb-8 md:mb-12">
        <div
          ...
          :class="isFilterOpen ? 'max-h-[800px] mb-6' : 'max-h-0 lg:max-h-none lg:mb-0'"
        >
          <div class="bg-white rounded-lg shadow-sm p-4 md:p-6 lg:sticky lg:top-8">
            ...
            <div class="space-y-1 md:space-y-2">
              <button
                ...
                :class="{
                  'bg-[#90a88c]/10 text-[#90a88c] font-semibold': !selectedCategory,
              ...
              >
                ...
                <span class="ml-auto text-xs px-1.5 py-0.5 md:px-2 md:py-1 bg-gray-100 rounded-full">
                  {{ allAdvice.length }}
                </span>
              </button>
              ...
              <button
                ...
                :class="{
                  'bg-[#90a88c]/10 text-[#90a88c] font-semibold': selectedCategory === category.id,
                  'hover:bg-gray-50 text-gray-700': selectedCategory !== category.id
                }"
              >
                ...
                <span class="text-xs px-1.5 py-0.5 md:px-2 md:py-1 bg-gray-100 rounded-full">
        ...
        <div class="lg:w-3/4">
          <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-4 md:mb-6 gap-3 md:gap-4">
            <h2 class="font-poppins font-bold text-lg md:text-xl text-[#1e1e1e]">
              {{ selectedCategory ? `Porady dla: ${getCategoryName(selectedCategory)}` : 'Wszystkie porady' }}
              <span v-if="filteredAdvice.length > 0" class="text-gray-600 font-normal text-xs md:text-sm ml-1 md:ml-2">
          ...
          <div v-if="filteredAdvice.length > 0" class="grid grid-cols-2 md:grid-cols-2 gap-3 sm:gap-4 md:gap-6">
            <div
            ...
            >
              ...
              <div class="p-3 sm:p-4 md:p-6">
                ...
                <div class="mb-2 md:mb-4">
                  <span class="text-[10px] sm:text-xs px-1.5 sm:px-2 py-0.5 sm:py-1 bg-gray-100 text-gray-600 rounded">
                ...
                <div class="space-y-1.5 sm:space-y-2 mb-3 md:mb-4">
                  <div class="flex items-start text-gray-600">
                    <svg class="w-3 h-3 sm:w-4 sm:h-4 text-[#90a88c] mr-1.5 sm:mr-2 mt-0.5 flex-shrink-0" fill="currentColor" viewBox="0 0 24 24">
                      <path d="M6 2L3 6v14c0 1.1.9 2 2 2h14c1.1 0 2-.9 2-2V6l-3-4H6zm0 2h12l1.5 2.5L18 8H6l-1.5-2.5L6 4zm0 4v10h12V8H6z"/>
                    </svg>
                    <span class="text-xs sm:text-sm line-clamp-2">{{ advice.light }}</span>
                  </div>
                  <div class="flex items-start text-gray-600">
                    <svg class="w-3 h-3 sm:w-4 sm:h-4 text-[#90a88c] mr-1.5 sm:mr-2 mt-0.5 flex-shrink-0" fill="currentColor" viewBox="0 0 24 24">
                    ...
                    <span class="text-xs sm:text-sm line-clamp-2">{{ advice.watering }}</span>
                  </div>
                </div>
                ...
                <div class="mb-3 md:mb-4">
                  ...
                  <ul class="text-[10px] sm:text-xs text-gray-600 space-y-0.5 sm:space-y-1">
                    ...
                    >
                      <svg class="w-2.5 h-2.5 sm:w-3 sm:h-3 text-[#90a88c] mr-1 sm:mr-1.5 mt-0.5 flex-shrink-0" fill="currentColor" viewBox="0 0 20 20">
          ...
          <div v-else class="text-center py-8 md:py-12 bg-white rounded-lg border border-gray-200">
            <div class="flex justify-center mb-3 md:mb-4">
              <svg class="w-12 h-12 md:w-16 md:h-16 text-gray-300" fill="currentColor" viewBox="0 0 24 24">
                <path d="M12 2L3 9v11h6v-7h6v7h6V9l-9-7z"/>
              </svg>
            </div>
            <p class="text-gray-500 text-base md:text-lg mb-2">Nie znaleziono porad spełniających kryteria</p>
            <button
              @click="clearFilter"
      ...
      <div id="faq" class="mt-8 md:mt-12 pt-6 md:pt-8 border-t border-gray-200">
        <div class="max-w-5xl mx-auto">
          <h2 class="font-poppins font-bold text-lg md:text-xl text-[#1e1e1e] mb-4 md:mb-6">
          ...
          <div class="space-y-3 md:space-y-4">
            ...
            >
              <div class="flex items-start">
                <div class="flex-shrink-0 w-6 h-6 md:w-8 md:h-8 bg-[#90a88c] text-white rounded-full flex items-center justify-center mr-3 md:mr-4">
                  <span class="font-semibold text-xs md:text-sm">?</span>
                </div>
                <div>
                  <h3 class="font-semibold text-gray-900 text-sm md:text-base mb-1 md:mb-2">{{ faq.question }}</h3>
                  <p class="text-gray-600 text-xs md:text-sm">{{ faq.answer }}</p>
                </div>
              </div>
            </div>
          </div>
...
const sortBy = ref<'easy-first' | 'hard-first' | 'alphabetical' | 'alphabetical-desc'>('easy-first')
...
const faqs = ref<FAQ[]>([
  ...
  },
  {
    ...
    answer: 'Żółknięcie liści może oznaczać przelanie, niedобór składników odżywczych lub zbyt mało światła. Dostosuj pielęgnację do potrzeb rośliny.'
  },
  {
    question: 'Kiedy przesadzać rośliny?',
    answer: 'Rośliny przesadzamy wiosną, gdy korzenie przerastają doniczkę. Zazwyczaj co 1-3 lat w zależności od tempa wzrostu.'
  },
  {
    question: 'Jakie rośliny są najlepsze dla początkujących?',
    answer: 'Dla początkujących polecamy zamiokulkasy, sansewierie, sukulenty i некоторые gatunki fikusów - są wytrzymałe i wybaczają błędy w pielęgnacji.'
  },
  {
    question: 'Czy wszystkie rośliny potrzebują dużo światła?',
    answer: 'Nie, zapotrzebowanie na światło różni się w zależności od gatunku. Niektóre rośliny (jak zamiokulkas) dobrze radzą sobie w półcieniu.'
  }
])
...
const filteredAdvice = computed(() => {
  ...
  if (selectedDifficulty.value !== 'all') {
    advice = advice.filter((a: CareAdvice) => a.difficulty === selectedDifficulty.value)
  }
  ...
  if (sortBy.value === 'easy-first') {
    ...
    advice.sort((a: CareAdvice, b: CareAdvice) => b.title.localeCompare(a.title))
  }
